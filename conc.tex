...
Currently available \ac{NGS} sequencing platforms are shown in \tabreft{tab:seq_tech} comparing sequence length, throughput and error rates.
Clearly, there are trade-offs between throughput per run and sequencing lengths with higher read depth technologies generally offering shorter read lengths, while the long read lengths offered by single-molecular sequencing (Pacific Biosciences) is off-set by high error-rate.
\input{conc_figures/seq_tech.tex}

\subsubsection{introduce computational limitations in computing power}
...
Local cluster computing has thus far been the standard way to process metagenomic datasets. 
However, acquiring the necessary computational resources can work out more costly for groups expecting only sporadic use and the time taken to process massive datasets can be prohibitively slow.
For example, %THe metagenomic vs moore's law says it will take decades with standard pipeline to analyse illumina data. Find better citation that says the same thing. Maybe Wilkening paper.

\subsubsection{webservers are useful tools and they can make up some of the shortfall in computing power}
This can been addressed to some extent by the availibility of webserver pipelines specialised in metagenomic processing such as \ac{MG-RAST} \cite{Meyer2008}, \ac{CAMERA} \cite{Sun2011} and \ac{IMG/M} \cite{Markowitz2008, Markowitz2012}, as well as \textsc{Metavir} \cite{Roux2011} and \textsc{VIROME} \cite{Wommack2012} for viral metagenomes.
These webservers take on the burden of maintaining necessary programs and hosting comparative sequence databases as well as providing computational power.
The web browser interface makes them user-friendly and each have their own features, such as particular analytical or visualisation tools, that may be more desirable for particular sample types or analyses.
However, these workflows are generally not customisble if the user's needs fall too far outside of what is provided.
For example, current webservers have been designed towards microbial or viral sequences, although metagenomes such as from the studies in this thesis often contain significant proportions of both.
To take advantage of the range of specialist features offered by different webservers entails repeating analytical steps, such as sequence filtering, in order to fit within the different analytical schema and then facing the difficulty of combining outputs.
Another example is that pipelines are often designed for particular sequencing technology so handling of other types may not be well-supported.
Furthermore, processing massive datasets may still take an unfeasibly long time depending on the capabilites of the webserver.
%Do any support illumina data?

\subsubsection{Cloud computing can make up shortfall in limited computing power}
Cloud computing resources look to be a promising way to make up the shortfall in computational power needed.
Custom bioinformatic workflows can be implemented depending on the context of the project, but generally require more computational expertise to set up than webservers.
...
%Give examples %Wilkening case study metagenomics
%the shortfall requires smarter ways to process.

\subsubsection{Computing tools need to be and are being developed to cope with the new sequences}
...
\subsubsection{Metagenomes and metaproteomes are a repository for prosterity}
