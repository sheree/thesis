...
The potential for carbon, nitrogen and sulphur conversions determined for the Organic Lake summer profile can be corroborated by functional analyses.
A matched metaproteomic analysis from the same samples has not yet been performed. 
Detection of proteins for the pathways identified in the metagenome can confirm that these genes are expressed and are therefore active.

However, for certain pathways, determining the rates of reaction are important.
Test in culture.
For the carbon cycle, it was hypothesized that active \ac{AAnP} and rhodopsin mediated photoheterotrophy reduced overall consumption of organic carbon.
To be able to test this in culture, carbon usage of isolates grown in the presence and absence of light can be used to determine if photoheterotrophy occurs.
The metaproteomic analysis can see if these proteins are expressed in the two conditions.
To test rhodopsin, look up the test for ion pumping.
Another indicator of \ac{AAnP} is \ac{BchlA} production.

Things that can be tested with in situ rates
denitrification, nitrogen fixation
In both these cases, genes were detected and it was inferred that rates of reaction were slow, but this may not be the case.
sulphate-reduction and oxidation.


Who is doing what the most?
Things that can be tested with SIP.
Test DMS usage in the bottom samples and top samples.
Test who takes up DMSP, who are the first responders.
...
Thing that need some molecular confirmation.
DMSP lyases can be synthesized and cloned
As can the rhodopsins

%%Inputs into the lake.
Look for inputs from melt streams.
Sample the chemical composition of inputs and see if they are enriched in say nitrates. 
Thid would challenge our assumptions of the carbon and nitrogen cycles.
