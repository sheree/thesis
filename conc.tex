...
Other complementary techniques, such as single cell and virus genomics and \ac{SIP}, hold promise for learning about specific populations in the community.
...
Metagenomic sequencing in a future summer season would provide data spanning several years thereby detecting interannual similarities and differences.
...
These two virophage systems in culture are associated with heterophic protists making the \ac{OLV} the only genomic sequence currently available for virophage affecting cosmopolitan phytoplankton species.
...
In conjunction with metagenomic sequencing, or flow sorting and \acp{SAG} of \ac{DMS}/\ac{DMSP}-degraders, the biochemical pathways involved in \ac{DMS} breakdown can then be reconstructe giving a more complete understanding sulphur biogeochemistry.
...
For example, detection of the \ac{OLV} was contingent on the addition of a sufficiently close relative, Sputnik, to the \ac{NR}.
...
As mentioned above, future `-omic' studies need to progress to encompass longer time scales, and finer resolution of the populations and individuals within them.
...
The hypotheses built on environmental data can then guide laboratory research studies and thus feedback into supporting systems-level understanding of microbial communities.
...
This thesis has demonstrated how metagenomics and metaproteomics can be used as a tool for discovery and characterising Antarctic lake ecosystems.
The \textsc{DNA} sequence and mass spectra data that has been produced form a lasting record of the state of the microbial community at that point in time.
