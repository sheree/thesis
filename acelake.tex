...
Since spectral matching depends on extremely high mass sensitivity, the ideal genomic database contains complete coverage of sequences from the organism(s) from which the proteins originated to get the most identifications. 
...
The use of a cross-species database requires the genomes to be sufficiently related to identify proteins balanced with stringent statistial cut-off to avoid false-positive matches.
...
The low identification rate is to be expected as it has been shown that only half the number of proteins are identifiable when using a genomic database that shares 90\% amino acid identity with the matching genome \cite{Denef2007}.
...
The finaly analysis workflow was applied to all the Ace Lake metaproteomic mass specta datasets.
...
This gave an indication of how functional processes are stratified with depth.
...
The prevalence of amino acid and simple sugar transporters \tabref{tab:ace_protids_5m}\tabref{tab:ace_protids_11.5m}, and the low \ac{DOC} concentration in the Ace Lake mixolimnion \figref{fig:ace_diagram} is likely to reflect efficient utilisation of these substrates from the \ac{DOC} pool.
...
The known genome streamlining of SAR11 \cite{Giovannoni2005} coupled with this similar expression pattern of SAR11 transporters from very environments is consistent with most transporters having a broad substrate range.
Two SAR11 transport proteins that were detected in Ace Lake \tabref{tab:ace_protids_5m}\tabref{tab:ace_protids_11.5m} were not detected in the Sargasso Sea \cite{Sowell2009}: an ectoine/hydroxyectoine (167807477 and 167892279) and a zinc \ac{ABC} transporter (167933120). 
Ectoine is a compatible solute and presence of the transporter which may be more availible in Ace Lake than in the ocean.
...
\emph{Actinobacteria} sequences were associated with a diverse phylogenetic cluster (Luna cluster) mainly contributed by freshwater ultramicrobacteria \cite{Hahn2003}. 
...
This was the first time these actinorhodopsin sequences have been found to be expressed.
The abundance of \emph{Actinobacteria} transporters along with its small cell size and distribution in the water column indicates it occupies a similar ecological niche as SAR11.
SAR11 contains proteorhodopsin which is a related, but phylogenetically distinct from actinorhodopsin \cite{Sharma2009}.
...
This indicates translational machinery is being upregulated on a translational level.

Metagenomic supported by microscopic analysis of the \ac{GSB} layer has indicated the population is free of virus predation.
Mathematical modeling predicted that the absence of virus predation in the \ac{GSB} could be due to an adaptation to longer cycles of growth and inactivity in response to the polar light regime \cite{Lauro2011}. 
